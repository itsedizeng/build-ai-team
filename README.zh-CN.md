...
Build AI Team 是一个完全开源、以 Codex 为核心的 AI 团队规划 Skill。它会根据真实的交付、责任、权限、独立验证和容量需求，判断一个 Agent 是否足够，或者是否值得建立多个长期 Agent；同时给出角色分工、负责结果、模型配置、本地适用 Skill，并在用户要求时搜索公开来源中的外部 Skill。
...
- 按角色主体工作推荐一个默认模型与推理强度；只有确有价值时才补充升级条件或更低成本的模型配置。
- 推荐最多三个本地适用 Skill，保留完整标识并准确说明状态。用户明确要求后，才单独搜索公开来源中的外部 Skill，并检查适用性、来源、维护信号和权限风险。
- 用户确认多 Agent 建队后，在适用时将当前规划对话复用为承担项目主控职责的实质角色，只新建其他当前必要成员；成员标题只使用角色名。
- 以真实长期任务作为当前团队状态，不创建或维护成员清单、计划或团队状态文档。
- 将方案确认、建队、公开 Skill 搜索、安装、登录、私有数据访问、写入、上传、发布和删除分成不同动作。
